...
# 每天选择收盘价最低的20个股票，进行轮动
def downcast(amount, lot):
    return abs(amount // lot * lot)


class LowPriceStrategy(bt.Strategy):
    params = (
        ('maperiod', 20),
    )
        ...
        if len(self.buy_list) < 2:  # 没有持仓
            ...
            for secu in set(self.getdatanames()) - set(self.buy_list):
                ...
                if data.close > self.sma[secu] and pd.Timestamp(data.datetime.date(1)) < end_dates[secu]:
                    ...
                    order_amount = downcast(order_value / data.close[0], 100)
...
cerebro.addstrategy(LowPriceStrategy, maperiod=20)
cerebro.run()
# 获取回测结束后的总资金
